...
export function useResearchPolling({ workflowId, documentId, onUpdate, enabled = true }: ResearchPollingOptions) {
  ...
  useEffect(() => {
    ...
    if (!enabled || !workflowId || !documentId || completedRef.current) {
      console.log('❌ POLLING HOOK - Not starting polling due to missing requirements');
      return;
    }
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.completed || data.error) {
            ...
            console.log('- Reason:', data.completed ? 'Workflow completed' : 'Workflow error');
            console.log('- Final Statistics:', data.statistics || 'No statistics');
            ...
            console.log('Research completed:', data.statistics || 'No statistics available');
    ...
    intervalRef.current = setInterval(pollWorkflow, 10000);
    ...
    // Cleanup on unmount
    return () => {
      console.log('🧹 POLLING CLEANUP - Component unmounting, clearing interval');
      if (intervalRef.current) {
        clearInterval(intervalRef.current);
        intervalRef.current = null;
      }
