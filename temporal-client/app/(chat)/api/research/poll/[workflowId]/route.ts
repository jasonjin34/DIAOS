...
// No database imports needed since we're not persisting documents
  ...
  try {
    ...
    if (!workflowId || !documentId) {
      return Response.json(
        { error: "Workflow ID and Document ID are required" },
        { status: 400 }
      );
    }
    ...
    if (workflowInfo.status.name === "COMPLETED") {
      try {
        ...
        if (result && result.success) {
          // Use default research topic since we're not persisting documents
          const researchTopic = "Research Topic";
...
## Key Papers Found
${
  result.research_context?.discovered_papers
    ?.slice(0, 5)
    .map(
...
## Detailed Findings
${
  result.research_context?.tool_results
    ?.map(
...
${
  result.research_context?.citations
    ?.slice(0, 10)
    .map(
...
*Research completed using Temporal workflows with comprehensive academic analysis and citation network mapping.*`;
          ...
          // Research completed - return results without database persistence
          response.completed = true;
          response.documentUpdated = false; // Not saving to database
          response.statistics = {
